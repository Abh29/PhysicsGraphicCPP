...
public:
  ...
  using raw_ptr = Contact *;
  /**
   * Holds the bodies that are involved in the contact. The
   * second of these can be NULL, for contacts with the scenery.
   */
  RigidBody::raw_ptr _body[2];
...
protected:
   ...
   * function is run.
   */
  glm::vec3 _relativeContactPosition[2];
...
protected:
   ...
   */
  glm::vec3 calculateLocalVelocity(unsigned bodyIndex, real_t duration);
  ...
  void applyPositionChange(glm::vec3 linearChange[2],
                           glm::vec3 angularChange[2], real_t penetration);
...
public:
   ...
   */
  ContactResolver(unsigned iterations, real_t velocityEpsilon = (real_t)0.01,
   ...
   */
  bool isValid() {
    return (_velocityIterations > 0) && (_positionIterations > 0) &&
           (_positionEpsilon >= 0.0f) && (_positionEpsilon >= 0.0f);
  }
   ...
   */
  void setEpsilon(real_t velocityEpsilon, real_t positionEpsilon);
   ...
   */
  void resolveContacts(Contact *contactArray, unsigned numContacts,
                       real_t duration);
...
protected:
   ...
   */
  void adjustVelocities(Contact *contactArray, unsigned numContacts,
                        real_t duration);
