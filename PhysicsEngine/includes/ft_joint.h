...
public:
   ...
   */
  void set(RigidBody::raw_ptr a, const glm::vec3 &a_pos, RigidBody::raw_ptr b,
           const glm::vec3 &b_pos, real_t error);

  /**
   * Generates the contacts required to restore the joint if it
   * has been violated.
   */
  unsigned addContact(Contact::raw_ptr contact, unsigned limit) const;
};
