...
ft::Buffer::Buffer(std::shared_ptr<Device> &device, VkDeviceSize size,
                   VkBufferUsageFlags usageFlags,
                   VkMemoryPropertyFlags memoryProperties, bool mapped,
                   VkDeviceSize mappedOffset, VkMemoryMapFlags mappedFlags)
  ...
  if (vkCreateBuffer(_ftDevice->getVKDevice(), &bufferCreateInfo, nullptr,
                     &_buffer) != VK_SUCCESS) {
  ...
  vkGetBufferMemoryRequirements(_ftDevice->getVKDevice(), _buffer,
                                &memoryRequirements);
  ...
  memoryAllocateInfo.memoryTypeIndex = _ftDevice->findMemoryType(
  ...
  if (_isMapped)
    vkMapMemory(_ftDevice->getVKDevice(), _bufferMemory, mappedOffset, _size,
                mappedFlags, &_mappedData);
}
...
void *ft::Buffer::getMappedData() const { return _mappedData; }
...
void ft::Buffer::copyToBuffer(ft::Buffer::pointer &dst, VkDeviceSize size,
  ...
  vkCmdCopyBuffer(commandBuffer->getVKCommandBuffer(), _buffer, dst->_buffer, 1,
                  &copyRegion);
  commandBuffer->end();
  commandBuffer->submit(_ftDevice->getVKGraphicsQueue());
}

bool ft::Buffer::isMapped() const { return _isMapped; }
...
void ft::Buffer::copyToImage(ft::Image::pointer &image, uint32_t width,
  ...
  vkCmdCopyBufferToImage(
      commandBuffer->getVKCommandBuffer(), _buffer, image->getVKImage(),
      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, copyRegions.data());

  commandBuffer->end();
  commandBuffer->submit(_ftDevice->getVKGraphicsQueue());
}

void ft::Buffer::copyToImage2(const Image::pointer &image, uint32_t width,
                              uint32_t height, uint32_t mipLevels,
                              uint32_t layers, uint32_t offsetsSize,
                              size_t *offsets) {
  ...
  for (uint32_t l = 0; l < layers; l++) {
    for (uint32_t i = 0; i < mipLevels; ++i) {
      VkBufferImageCopy region{};
      region.bufferOffset =
          offsets ? offsets[std::max((int)offsetsSize - 1, off++)] : 0;
      region.bufferRowLength = 0;
      region.bufferImageHeight = 0;
      region.imageSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
      region.imageSubresource.mipLevel = i;
...
void ft::Buffer::copyFromImage(const Image::pointer &image,
  ...
  vkCmdCopyImageToBuffer(commandBuffer->getVKCommandBuffer(),
                         image->getVKImage(), layout, _buffer, 1, &region);
...
ft::BufferBuilder &
ft::BufferBuilder::setUsageFlags(VkBufferUsageFlags usageFlags) {
  _usageFlags = usageFlags;
  return *this;
}

ft::BufferBuilder &
ft::BufferBuilder::setMemoryProperties(VkMemoryPropertyFlags memoryProperties) {
...
ft::Buffer::pointer ft::BufferBuilder::build(std::shared_ptr<Device> &device) {
  _ftBuffer =
      std::make_unique<Buffer>(device, _size, _usageFlags, _memoryProperties,
...
ft::BufferBuilder &
ft::BufferBuilder::setMappedOffset(VkDeviceSize mappedOffset) {
  _mappedOffset = mappedOffset;
  return *this;
}

ft::BufferBuilder &
ft::BufferBuilder::setMappedFlags(VkMemoryMapFlags mappedFlags) {
