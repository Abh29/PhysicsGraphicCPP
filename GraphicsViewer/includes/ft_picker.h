...
public:
  ...
  MousePicker(Device::pointer device, uint32_t width, uint32_t height,
              PickingRdrSys::pointer rdrSys);
  ~MousePicker();

  [[nodiscard]] uint32_t pick(const Scene::pointer &scene, uint32_t x,
                              uint32_t y);
