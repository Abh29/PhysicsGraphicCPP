...
//		inputAssemblyStateCreateInfo.topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
		...
		void createRenderPass() {
			// attachment description for color
			VkAttachmentDescription colorAttachmentDescription{};
			colorAttachmentDescription.format = _ftSwapChain->getVKSwapChainImageFormat();
			colorAttachmentDescription.samples = _ftDevice->getMSAASamples();
			...
			// attachment description for multisampling resolve
			VkAttachmentDescription colorAttachmentResolve{};
			colorAttachmentResolve.format = _ftSwapChain->getVKSwapChainImageFormat();
			...
			// attachment description for depth
			VkAttachmentDescription depthAttachmentDescription{};
			depthAttachmentDescription.format = _ftDevice->findDepthFormat();
			depthAttachmentDescription.samples = _ftDevice->getMSAASamples();
			...
			// sub passes and attachment references
			VkAttachmentReference colorAttachmentReference{};
			colorAttachmentReference.attachment = 0;
			colorAttachmentReference.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

			VkAttachmentReference colorAttachmentResolveReverence{};
			colorAttachmentResolveReverence.attachment = 2;
			colorAttachmentResolveReverence.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
			...
			subpassDescription.pResolveAttachments = &colorAttachmentResolveReverence;
			...
			if (vkCreateRenderPass(_ftDevice->getVKDevice(), &renderPassCreateInfo, nullptr, &_renderPass) != VK_SUCCESS) {
		...
		void drawFrame() {
			...
			uint32_t imageIndex;
			VkResult result = vkAcquireNextImageKHR(_ftDevice->getVKDevice(), _ftSwapChain->getVKSwapChain(), UINT64_MAX, _imageAvailableSemaphores[_currentFrame], VK_NULL_HANDLE, &imageIndex);
			...
			recordCommandBuffer(_ftCommandBuffers[_currentFrame], imageIndex);
			...
			result = vkQueuePresentKHR(_ftDevice->getVKPresentQueue(), &presentInfoKhr);
		...
		void createVertexBuffer() {
			...
			// copy the data
			copyBuffer(stagingBuffer->getVKBuffer(), _ftVertexBuffer->getVKBuffer(), bufferSize);
		}

		void copyBuffer(VkBuffer srcBuffer, VkBuffer destBuffer, VkDeviceSize size) {
			// create a tmp command buffer for the copy operation and begin recording
			...
			vkCmdCopyBuffer(commandBuffer->getVKCommandBuffer(), srcBuffer, destBuffer, 1, &copyRegion);

			// end recording and execution with clean up
			commandBuffer->end();
		...
		void createIndexBuffer() {
			...
			// copy buffer
			copyBuffer(stagingBuffer->getVKBuffer(), _ftIndexBuffer->getVKBuffer(), bufferSize);
		}
		...
		void createTextureImage() {
			...
			copyBufferToImage(stagingBuffer->getVKBuffer(), _ftTextureImage->getVKImage(), texWidth, texHeight);
		...
		void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
			...
			// specify the regions to be copied and add the copy command
