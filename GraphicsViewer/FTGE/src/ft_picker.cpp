...
ft::MousePicker::MousePicker(Device::pointer device, uint32_t width,
                             uint32_t height, ft::PickingRdrSys::pointer rdrSys)
...
void ft::MousePicker::createPickingRenderPass() {
  ...
  ft::Attachment::pointer colorAttachment =
      attachmentBuilder.setDescriptionFormat(VK_FORMAT_R32_UINT)
  ...
  ft::Attachment::pointer depthAttachment =
      attachmentBuilder.setDescriptionFormat(_ftDevice->findDepthFormat())
          .setDescriptionSamples(VK_SAMPLE_COUNT_1_BIT)
          .setDescriptionLoadOp(VK_ATTACHMENT_LOAD_OP_CLEAR)
          .setDescriptionStoreOp(VK_ATTACHMENT_STORE_OP_DONT_CARE)
          .setDescriptionFinalLayout(
              VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL)
          .setReferenceImageLayout(
              VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL)
          .build();

  ft::SubPass::pointer subPass =
      std::make_shared<ft::SubPass>(VK_PIPELINE_BIND_POINT_GRAPHICS);
  ft::SubpassDependency::pointer subpassDependency =
      std::make_shared<ft::SubpassDependency>(VK_SUBPASS_EXTERNAL, 0);

  subpassDependency
      ->setSrcStageMask(VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT |
                        VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT)
      .setSrcAccessMask(VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT |
...
void ft::MousePicker::createPickingResources() {
  ...
  _ftDepthImage =
      imageBuilder.setWidthHeight(_width, _height)
          .setMipLevel(1)
          .setSampleCount(VK_SAMPLE_COUNT_1_BIT)
  ...
  Image::transitionImageLayout(_ftDevice, _ftDepthImage->getVKImage(),
  ...
  if (vkCreateFramebuffer(_ftDevice->getVKDevice(), &framebufferCreateInfo,
                          nullptr, &(_pickingFrameBuffer)) != VK_SUCCESS) {
  ...
  _ftColorBuffer =
      bufferBuilder.setIsMapped(true)
...
void ft::MousePicker::drawIDs(const ft::Scene::pointer &scene) {
  ...
  if (vkBeginCommandBuffer(_ftCommandBuffer->getVKCommandBuffer(),
                           &commandBufferBeginInfo) != VK_SUCCESS) {
  ...
  vkCmdBeginRenderPass(_ftCommandBuffer->getVKCommandBuffer(),
                       &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);

  // bind the graphics pipeline
  vkCmdBindPipeline(_ftCommandBuffer->getVKCommandBuffer(),
                    VK_PIPELINE_BIND_POINT_GRAPHICS,
                    _ftPickingRdrSys->getGraphicsPipeline()->getVKPipeline());

  // bind the descriptor sets
  vkCmdBindDescriptorSets(
      _ftCommandBuffer->getVKCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS,
      _ftPickingRdrSys->getGraphicsPipeline()->getVKPipelineLayout(), 0, 1,
      &(_ftPickingRdrSys->getDescriptorSets()[0]->getVKDescriptorSet()), 0,
      nullptr);
  ...
  viewport.x = 0.0f;
  viewport.y = 30.0f;
  viewport.width = static_cast<float>(_width - 200);
  ...
  scene->drawPickObjs(_ftCommandBuffer, _ftPickingRdrSys->getGraphicsPipeline(),
                      0);
  ...
  // end command buffer
  if (vkEndCommandBuffer(_ftCommandBuffer->getVKCommandBuffer()) !=
      VK_SUCCESS) {
  ...
  if (vkQueueSubmit(_ftDevice->getVKGraphicsQueue(), 1, &submitInfo, _fence) !=
      VK_SUCCESS) {
    throw std::runtime_error("failed to submit a draw command buffer!");
  }

  Image::transitionImageLayout(_ftDevice, _ftColorImage->getVKImage(),
                               VK_FORMAT_R32_UINT,
                               VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                               VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 1);
  _ftColorBuffer->copyFromImage(_ftColorImage,
...
void ft::MousePicker::createPickingSyncObjs() {
  ...
  fenceCreateInfo.flags =
      VK_FENCE_CREATE_SIGNALED_BIT; // this is for the first call

  if (vkCreateFence(_ftDevice->getVKDevice(), &fenceCreateInfo, nullptr,
                    &_fence) != VK_SUCCESS)
