...
VkPipelineLayout ft::GraphicsPipeline::getVKPipelineLayout() const {return _pipelineLayout;}

void ft::GraphicsPipeline::createPipeline(PipelineConfig &config) {
	_ftVertexShader = std::make_shared<ft::Shader>(_ftDevice, config.vertShaderPath, ft::Shader::ShaderType::FT_VERTEX_SHADER);
	_ftFragmentShader = std::make_shared<ft::Shader>(_ftDevice, config.fragShaderPath, ft::Shader::ShaderType::FT_FRAGMENT_SHADER);
	...
	dynamicStateCreateInfo.dynamicStateCount = static_cast<uint32_t>(config.dynamicStates.size());
	dynamicStateCreateInfo.pDynamicStates = config.dynamicStates.data();

	// vertex input state
	VkPipelineVertexInputStateCreateInfo vertexInputStateCreateInfo{};
	vertexInputStateCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
	vertexInputStateCreateInfo.vertexBindingDescriptionCount = static_cast<uint32_t>(config.bindings.size());
	vertexInputStateCreateInfo.pVertexBindingDescriptions = config.bindings.data();
	vertexInputStateCreateInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(config.attributes.size());
	vertexInputStateCreateInfo.pVertexAttributeDescriptions = config.attributes.data();
	...
	// color blending
	config.colorBlendState.attachmentCount = static_cast<uint32_t>(config.colorBlendAttachments.size());
	...
	pipelineLayoutCreateInfo.pushConstantRangeCount = static_cast<uint32_t>(config.pushConstantRanges.size());
	pipelineLayoutCreateInfo.pPushConstantRanges = config.pushConstantRanges.data();
	...
	if (vkCreateGraphicsPipelines(_ftDevice->getVKDevice(), VK_NULL_HANDLE, 1, &pipelineCreateInfo, nullptr, &_pipeline) != VK_SUCCESS) {
