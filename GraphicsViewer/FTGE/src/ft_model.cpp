...
void ft::Model::bind(const ft::CommandBuffer::pointer& commandBuffer, uint32_t index) {
    (void) index;
...
void ft::Model::draw(const ft::CommandBuffer::pointer& commandBuffer, const GraphicsPipeline::pointer& pipeline) {
...
//                           &push);
//        vkCmdDraw(commandBuffer->getVKCommandBuffer(), static_cast<uint32_t>(_indices.size()), _node->state.instanceCount, 0, 0);
//    }
}
...
void ft::Model::loadModel() {
    ...
    _node->mesh.push_back({0, static_cast<uint32_t>(_indices.size()), 0, nullptr});
...
void ft::Model::writePerInstanceData(uint32_t index) {
	_ftInstanceBuffers[index]->copyToMappedData(_instances.data.data(), sizeof(InstanceData) * _instances.data.size());
}
    ...
    if (inputNode.mesh > -1) {
        ...
        for (size_t i = 0; i < mesh.primitives.size(); ++i) {
            ...
            if (p.attributes.find("POSITION") != p.attributes.end()) {
                const tinygltf::Accessor& accessor = input.accessors[p.attributes.find("POSITION")->second];
                const tinygltf::BufferView& view = input.bufferViews[accessor.bufferView];
                positionBuffer = reinterpret_cast<const float*>(&(input.buffers[view.buffer].data[accessor.byteOffset + view.byteOffset]));
                vertexCount = accessor.count;
            }
            if (p.attributes.find("NORMAL") != p.attributes.end()) {
                const tinygltf::Accessor& accessor = input.accessors[p.attributes.find("NORMAL")->second];
                const tinygltf::BufferView& view = input.bufferViews[accessor.bufferView];
                normalsBuffer = reinterpret_cast<const float*>(&(input.buffers[view.buffer].data[accessor.byteOffset + view.byteOffset]));

            }
            if (p.attributes.find("TEXCOORD_0") != p.attributes.end()) {
                const tinygltf::Accessor& accessor = input.accessors[p.attributes.find("TEXCOORD_0")->second];
                const tinygltf::BufferView& view = input.bufferViews[accessor.bufferView];
                texCoordsBuffer = reinterpret_cast<const float*>(&(input.buffers[view.buffer].data[accessor.byteOffset + view.byteOffset]));

            }
            if (p.attributes.find("TANGENT") != p.attributes.end()) {
                const tinygltf::Accessor& accessor = input.accessors[p.attributes.find("TAGENT")->second];
                const tinygltf::BufferView& view = input.bufferViews[accessor.bufferView];
...
bool ft::Model::hasFlag(uint32_t flag) const {return (_node->state.flags & flag);}
...
uint32_t ft::Model::addCopy(const ft::InstanceData &copyData) {
    if (!(_node->state.flags & ft::MODEL_SIMPLE_BIT)) return 0;
    uint32_t id = ID();
