...
ft::Renderer::Renderer(Window::pointer window, Surface::pointer surface,
                       PhysicalDevice::pointer physicalDevice,
                       Device::pointer device)
    : _ftWindow(std::move(window)), _ftSurface(std::move(surface)),
...
void ft::Renderer::initRenderer() {
  _ftSwapChain =
      std::make_shared<SwapChain>(_ftPhysicalDevice, _ftDevice, _ftSurface,
...
void ft::Renderer::endFrame(const CommandBuffer::pointer &commandBuffer,
                            uint32_t imageIndex) {
  // end command buffer
  commandBuffer->end();
...
void ft::Renderer::beginRenderPass(const CommandBuffer::pointer &commandBuffer,
                                   uint32_t imageIndex) {
  ...
  vkCmdBeginRenderPass(commandBuffer->getVKCommandBuffer(),
  ...
  viewport.y = 30.0f;
  viewport.width =
      static_cast<float>(_ftSwapChain->getVKSwapChainExtent().width - 200);
  viewport.height =
      static_cast<float>(_ftSwapChain->getVKSwapChainExtent().height);
...
void ft::Renderer::endRenderPass(const CommandBuffer::pointer &commandBuffer,
                                 uint32_t imageIndex) {
  (void)imageIndex;
  // render pass end
...
void ft::Renderer::recreateSwapChain() {
  ...
  _ftSwapChain.reset();
  _ftSwapChain = std::make_shared<SwapChain>(
      _ftPhysicalDevice, _ftDevice, _ftSurface,
      _ftWindow->queryCurrentWidthHeight().first,
      _ftWindow->queryCurrentWidthHeight().second, preferredMode);
  _ftSwapChain->createFrameBuffers(_ftRenderPass);
}
...
void ft::Renderer::initRenderPasses() {
  ...
  ft::Attachment::pointer colorResolver =
      attachmentBuilder
          .setDescriptionFormat(_ftSwapChain->getVKSwapChainImageFormat())
...
ft::SwapChain::pointer ft::Renderer::getSwapChain() const {
  return _ftSwapChain;
}

ft::RenderPass::pointer ft::Renderer::getRenderPass() const {
  return _ftRenderPass;
}
...
void ft::Renderer::createUniformBuffers() {
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    _ftUniformBuffers.push_back(
        _ftBufferBuilder->setSize(bufferSize)
...
std::vector<ft::Buffer::pointer> ft::Renderer::getUniformBuffers() const {
  return _ftUniformBuffers;
}
